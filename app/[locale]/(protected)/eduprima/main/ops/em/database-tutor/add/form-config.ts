...
export interface TutorFormData {
  ...
  transportasiTutor?: string[]; // Transportation methods (multi-select)
...
const validationRules = {
  ...
  trn: (value: string) => {
    if (!value) return null; // Allow empty values - will be auto-generated by database
    if (!/^[A-Z0-9]+$/.test(value)) return 'ERN harus berupa huruf kapital dan angka tanpa spasi';
    if (value.length < 8) return 'ERN minimal 8 karakter';
    return null;
  },
  ...
  steps: [
    ...
    
    {
      ...
      fields: [
        ...
        {
          name: 'transportasiTutor',
          label: 'Transportasi Mengajar',
          type: 'checkbox',
          required: false,
          multiple: true,
          icon: 'ph:car',
          placeholder: 'Pilih transportasi yang digunakan untuk mengajar',
          helperText: 'Pilih semua jenis transportasi yang Anda gunakan untuk mengajar (dapat memilih lebih dari satu)',
          size: 'lg',
          options: [
            { value: 'sepeda_motor', label: 'Sepeda Motor' },
            { value: 'online_transport', label: 'Online Transport (Gojek, Grab, dll)' },
            { value: 'transportasi_umum', label: 'Transportasi Umum (Bus, Angkot, KRL, dll)' },
            { value: 'sepeda', label: 'Sepeda' },
            { value: 'jalan_kaki', label: 'Jalan Kaki' },
            { value: 'mobil_pribadi', label: 'Mobil Pribadi' },
            { value: 'online_only', label: 'Online Saja (Tidak Perlu Transportasi)' }
          ]
        },
        {
        ...
        },
        {
          ...
          required: true,
          options: [
            { value: 'tutor_visit', label: '🏠 Privat Guru Ke Tempat Siswa (Tutor Visit)' },
            { value: 'student_visit', label: '🏢 Privat di Lokasi Guru' },
            { value: 'online_class', label: '💻 Privat Online' },
            { value: 'group_class', label: '👥 Bimbel Kelas' },
            { value: 'hybrid', label: '🔄 Kombinasi (Online + Offline)' },
...
export const defaultFormData: Partial<TutorFormData> = {
  ...
  transportasiTutor: [], // Transportation methods array
